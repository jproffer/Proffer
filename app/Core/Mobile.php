...
class Mobile {
	...
	public function __construct() {
		...
		if (strpos(strtolower($_SERVER['ALL_HTTP']),'operamini')>0) { $this->mobile=true; }
		if (strpos(strtolower($_SERVER['HTTP_USER_AGENT']),' ppc;')>0) { $this->mobile=true; }
		if (strpos(strtolower($_SERVER['HTTP_USER_AGENT']),'windows')>0) { $this->mobile=false; }
		if (strpos(strtolower($_SERVER['HTTP_USER_AGENT']),'iemobile')>0) { $this->mobile=true; }
	}
}
?>
