...
class lessc {
	...
	protected function newFormatter() {
		$className = "\library\lessc_formatter_lessjs";
		if (!empty($this->formatterName)) {
			if (!is_string($this->formatterName))
	return $this->formatterName;
			$className = "\library\lessc_formatter_$this->formatterName";
		}
...
class lessc_parser {
	...
	public function __construct($lessc, $sourceName = null) {
		...
		if (!self::$operatorString) {
			self::$operatorString =	'('.implode('|', array_map(array('\library\lessc', 'preg_quote'), array_keys(self::$precedence))).')';

			$commentSingle = \library\lessc::preg_quote(self::$commentSingle);
			$commentMultiLeft = \library\lessc::preg_quote(self::$commentMultiLeft);
			$commentMultiRight = \library\lessc::preg_quote(self::$commentMultiRight);
	 ...
	 *
	 * Consider the function \library\lessc::keyword(). (all parse functions are
	 * structured the same)
	 *
	 ...
	 *
	 * All of these parse functions are powered by \library\lessc::match(), which behaves
	 ...
	 * avoid this, \library\lessc::seek() is used to remember and set buffer positions.
	 *
	...
	protected function isDirective($dirname, $directives) {
		...
		$pattern = implode("|",
			array_map(array("\library\lessc", "preg_quote"), $directives));
	...
	protected function expressionList(&$exps) {
		...
		$exps = \library\lessc::compressList($values, ' ');
		return true;
	}
	...
	public function propertyValue(&$value, $keyName = null) {
		...
		$value = \library\lessc::compressList($values, ', ');
		return true;
	}
	...
	protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
		...
		$stop = array("'", '"', "@{", $end);
		$stop = array_map(array("\library\lessc", "preg_quote"), $stop);
	...
	protected function string(&$out) {
		...
		$patt = '([^\n]*?)(@\{|\\\\|' .
			\library\lessc::preg_quote($delim).')';
	...
	return false;
		...
		if (!isset(self::$literalCache[$what])) {
			self::$literalCache[$what] = \library\lessc::preg_quote($what);
		}
	...
	protected function to($what, &$out, $until = false, $allowNewline = false) {
		...
		if (!$this->match('('.$validChars.'*?)'.\library\lessc::preg_quote($what), $m, !$until)) return false;
